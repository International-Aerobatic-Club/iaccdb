...
module Iac::RegionalSeries

  include Iac::Region

  def required_contest_count(regional_pilot)
    if (regional_pilot.year < 2018)
      (/NorthWest/i =~ regional_pilot.region) ? 2 : 3;
    else
      3
    end
  end
  ...
  # Compute one competitor's results
  def compute_pilot_results(regional_pilot)
    best_pct = 0;
    best_combo = [];
    apc = regional_pilot.pc_results.all
    pc_ct = apc.count
    pc_req = required_contest_count(regional_pilot)
  ...
  # Other competitors will have a result in regions where they have
  ...
  def compute_results(year, region)
    ...
    nationals = Contest.where([
      'year(start) = ? and region = "National"',
      year
    ])
  ...
  def compute_ranking(year, region)
    ...
    cat_pilots.each do |_, pilots|
      ...
      pilots.each do |rp|
        rp.rank = rank
        rank = next_rank if (prev_qual != rp.qualified ||
